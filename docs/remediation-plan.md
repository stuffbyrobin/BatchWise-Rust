...
- [x] Same for `recipe/repository.rs` `update_calculations` and the four child `DELETE`s. Not done: the child tables have no `tenant_id`, and every caller runs them in the same transaction immediately after a tenant-scoped insert/update of the recipe. Low value; fold into Phase 8's recipe child-row rewrite. (Phase 8 scoped the DELETEs and `update_calculations`. Migration 35 then gave the four child tables a `tenant_id` with a composite foreign key to `recipes (id, tenant_id)`, so a child row can only carry its recipe's tenant; the child selects, inserts, deletes and the allergen lookup all filter on it.)
...
- [x] Guard the last Owner: an Owner cannot be demoted, deactivated or removed if no other Owner remains. (`PATCH /members/{id}` and `DELETE /auth/me` both refuse with 422 `last_owner`; the Owner rows are locked so two Owners cannot demote each other at once.)
- [x] Role changes write a compliance audit entry (actor, user, old role, new role). (`member.role_changed`, plus `member.deactivated` and `member.reactivated`. The members API is `GET /members` and `PATCH /members/{id}`: Owners manage everyone, Managers only Brewer, Sales and Viewer members. Deactivation revokes refresh tokens and every change applies from the member's next request. `is_owner` is dropped, migration `000033`.)
...
- [x] Frontend: return `role` from `/auth/me`; hide or disable actions the user cannot perform. The server stays the source of truth. (`auth/permissions.ts` mirrors the backend table, with a test on the same rows; the sidebar shows only what the role may open; a Members page manages roles. `RouteAccess` in the app shell shows "No access" for pages the role cannot open, including create and import pages without write access, and a read-only note on pages it can only read. Each page hides its create, edit and delete controls with `useCanWrite(area)`, or disables an always-visible edit form; label approval and spoiling a completed batch are shown only to Owners and Managers.)
...
- [x] Distributed rate limiting (Redis) once there is more than one replica. (Set `REDIS_URL` and every instance runs the same sliding-window Lua script against one Redis, keyed `<prefix>:rl:<limiter>:<caller>` and timed by Redis's own clock; the route limits and the per-account failed-login limit all go through it. Unset, each instance limits on its own as before, which is right for a single replica. The in-memory window keeps counting either way, so an unreachable Redis degrades to per-instance limiting instead of no limiting, and logs at most once a minute.)
- [x] Access-token revocation (`jti` denylist) if `JWT_EXPIRY_MINUTES` is ever raised above ~15. (Access tokens carry a `jti` and the user's `token_version`. Logout revokes the access token sent with it, by `jti`, until it expires. Password change, deactivation, account deletion and a replayed refresh token bump `token_version`, which revokes every session. Both are checked through the role cache, so revocation applies on the next request on the instance that handled it and within 30 seconds on any other. Tokens issued before this change have neither claim and are refused, so clients refresh once.)
