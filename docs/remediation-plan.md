...
- [ ] Frontend: return `role` from `/auth/me`; hide or disable actions the user cannot perform. The server stays the source of truth. (So far: `/auth/me` returns `role`; `auth/permissions.ts` mirrors the backend table, with a test on the same rows; the sidebar shows only what the role may open; and a Members page lists members, changes roles and deactivates. Hiding write actions on each page follows in 15c.)
